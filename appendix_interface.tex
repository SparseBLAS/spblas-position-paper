...
\end{minted}
\caption{Three-stage variant (with optional inspect phase) for the sparse matrix multiplication (SpGEMM) 
...
// csr_wrapper C is now ready to be used
\end{minted}
\caption{Two-stage variant (with optional inspect phase) for the sparse matrix multiplication (SpGEMM) 
$C = \alpha \cdot \text{op}(A) \cdot \text{op}(B) [ + \beta \cdot D ]$.}
\end{listing}
