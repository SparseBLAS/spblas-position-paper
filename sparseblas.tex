...
// to the matrix opt
multiply_inspect(policy, state, A_opt, x, y);

// actual multiplication y = A * x
multiply(policy, state, A_opt, x, y); 
...
multiply_inspect(policy, state, scaled(alpha, A_opt), X, scaled(beta, Y), Y);

// actual multiplication Y = alpha * A * X + beta * Y
multiply(policy, state, scaled(alpha, A_opt), X, scaled(beta, Y), Y); 
...
// solve
csr_wrapper<float> T(values, rowptr, colind, shape, nnz);
...
// matrix_opt is opaque and may contain
// vendor optimization details
auto T_handle = matrix_opt(T, allocator);
