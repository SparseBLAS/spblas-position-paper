...
Parameters for CSC and COO are given in \Cref{tab:csc_param} and \Cref{tab:coo_param}, respectively.
...
This concept of an optional inspect phase is in line with the current design of vendor interfaces, see~\Cref{sec:spmvReview}.
...
Operations with a known output size can be realized using a single routine. Functionality listed in~\Cref{tab:scope} falling into this category includes: scaling, transpose, and conjugate transpose, norms, sparse matrix vector multiplication, sparse matrix dense matrix multiplication, triangular solve, general, and sampled dense matrix multiplication. As previously mentioned, we allow for an optional \textit{inspect} phase for inspecting the sparse objects and passing information to the routines executing the operation. We present here the interface for the two archetypes producing known output size: the sparse matrix vector multiplication (\Cref{listing:spmv}) and the sparse triangular solve (\Cref{listing:sptrsv}). We list the interfaces to the other SparseBLAS functionality producing output of known size in the appendix.
...
can be realized using the three-stage API sequence shown in~\Cref{lst:spgemm_threestage}.
...
the simplified two-stage SpGEMM interface shown in~\Cref{lst:spgemm_twostage} can be used for Equation~\eqref{eq:SpGEMM}.
...
In the preceding sections, we have introduced a number of new types. \Cref{tab:newtypes} provides a summary.
...
%    we do not recommend it be included in a preliminary SparseBLAS standard.
    \item Run-to-run bitwise-reproducibility properties in SparseBLAS routines are discussed in~\Cref{sec:BitwiseReproducibility}. Note that the bitwise reproducibility property is orthogonal to consistency in exception propagation, so both may be required or not as applicable to the user.
...
many new algorithms mentioned in~\Cref{sec:ChoicesNumericalFormats},
and~\Cref{Error_Bounds},
