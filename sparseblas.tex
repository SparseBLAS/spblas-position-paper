...
As discussed, the non-owning approach limits the optimization potential for the libraries, as they can not handle sparse matrices in a hardware-optimized format of their choice or
...
With the intention of allowing for aggressive optimization while to a large degree preserving the concept of light-weight views, we suggest a two-level design where the first level is a ``view-like'' wrapper that encapsulates the user-owned raw data, and a \texttt{matrix\_opt} as the second level extends the matrix wrapper with a library-managed structure that can contain additional data for optimization.

The matrix wrapper -- we use the notation \texttt{xyz\_wrapper} where \texttt{xyz} encodes the sparse format, e.g., \texttt{csr\_wrapper} or \texttt{coo\_wrapper} -- wraps the user data and stores additional information about the matrix, including the matrix size and nonzero count, data types, index types, offsets, and property tags. Property tags can provide information about the structure, numerical symmetry, and sortedness of the entries in the respective format. We note that the sortedness property has different meanings for different matrix formats. We also note that although the \texttt{xyz\_wrapper} is motivated by the concept of a C++ view~\cite{cpplinalg:p1673r13}, it does not fulfill the property of the data being immutable, and we therefore refrain from using the term ``matrix view.'' For example, when creating a sparse matrix as the output of a function, the user-owned data structure is filled. Another example is functionality sorting the matrix values or reordering a matrix, thereby explicitly changing the user-owned data. Similarly, property tags can be added and changed by sparse BLAS functionality. For example, a sorting function changes the matrix tag, or an inspection function might identify a fill pattern as lower triangular. Users can also manually change matrix tags via member functions. 

To allow libraries for aggressive optimization, the format wrapper is encapsulated in a \texttt{matrix\_opt} that contains both the view to the user-owned raw data and a structure that can contain additional data for optimization. It is important that all the optimization data is owned by the library, not by the user, and the data is stored in library-allocated memory and is generally opaque to the user.

Some users want to keep control of all available memory, in particular, control how data structures are allocated. For this reason, we propose a design where the user can pass an \texttt{allocator} to the library that is then used by the library to allocate additional data in the \texttt{matrix\_opt}. If no allocator is provided, the library is permitted to use whichever allocator they deem to be appropriate.
...
\label{sec:views}
In some situations, a sparse matrix object is not used in its original form, but in a modified form, e.g., only a submatrix (lower triangular) is accessed, or the matrix in accessed in transposed form. In these scenarios, modifying the user-owned data in the \texttt{xyz\_wrapper} would conflict with the non-mutability of the user-owned data. Creating an explicit copy on the other hand would introduce a significant overhead. We resolve the situation by introducing the \texttt{view} instances shown in Table~\ref{tab:views}. Note that \verb|uplo| is one of \texttt{view::uplo::\{lower, upper\}}, and \texttt{diag} is one of \texttt{view::diag::\{explicit, implicit\_unit, implicit\_zero\}}.
...
\end{minted}
\caption{Sparse matrix vector product $y = \alpha \cdot A \cdot x\  [ + \beta \cdot z]$.}
\end{listing}
...
\end{minted}
\caption{Triangular solve: solve $x=\alpha A^{-1} b$.}
\end{listing}
...
with the addition of the sparse matrix $d$ being optional, can be realized using the following sequence of steps:
...
Finally, we note that this notion of conditional numerical reproducibility is  orthogonal to the notion of consistent exception handling discussed previously. Both can be set or not set independently. There is a small intersection, but 
\JimD{Should the following "The" just be "the"? The resulting sentence seems a
bit too long.}
