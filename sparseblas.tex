...
% minted v3 delegates to the latexminted Python executable. Use its bundled
% v2 compatibility layer when available; this also works with TeX installs in
% which latexminted itself is broken or unavailable. Older TeX releases only
% provide minted.sty, so retain that as a fallback.
\ifdefined\sbmintedfinalize
  \def\sbmintedmode{finalizecache}
\else
  \def\sbmintedmode{frozencache}
\fi
\edef\sbmintedopts{newfloat,\sbmintedmode,cachedir=_minted-cache}
\IfFileExists{minted2.sty}
  {\expandafter\usepackage\expandafter[\sbmintedopts]{minted2}}
  {\expandafter\usepackage\expandafter[\sbmintedopts]{minted}}
...
\makeatletter
 \AtBeginDocument{%
   \@ifundefined{hyxmp@parse@acmart}{}{%
     \let\SB@orig@hyxmp@parse@acmart\hyxmp@parse@acmart
     \def\hyxmp@parse@acmart{%
       \let\SB@saved@and\and
       \let\and\relax
       \SB@orig@hyxmp@parse@acmart
       \let\and\SB@saved@and}%
   }%
}
\makeatother
...
\definecolor{listing_bck}{HTML}{F2F2F2}
...
\author{Grace Dinh}
\authornote{Part of this research was conducted at the Simons Institute for the Theory of Computing, supported in part by ADIA Lab, Fall 2025.}
\affiliation{%
...
% Existing APIs are different but share certain design patterns, and we consider them as blueprints guiding the design of the new API standard for sparse linear algebra functionality. Thus, we consider it beneficial to the reader to list existing C and \cplusplus library APIs for the sparse matrix-vector (\texttt{SpMV}) operation multiplying a sparse matrix with a dense vector. Some APIs are synchronous and blocking, while others extend to asynchronous non-blocking execution models.
Existing APIs differ in their design, but share several common patterns that provide useful blueprints for the design of a new standard API for sparse linear algebra functionality. To illustrate these patterns, we review existing C and \cplusplus library APIs for the sparse matrix-vector (\texttt{SpMV}) operation, which computes the product of a sparse matrix and a dense vector. These APIs support different execution models: some are synchronous and blocking, whereas others additionally support asynchronous, non-blocking execution.
\Cref{tab:spmv_api_comparison} summarizes the primary differences across vendor and domain-specific APIs for double-precision Sparse Matrix-Vector Multiplication (\texttt{SpMV}), while \Cref{lst:spmv_comparison} in the Appendix provides code-level specifications.
...
\begin{table*}[b]
  \centering
  \caption{Comparison of execution APIs across vendor and domain libraries for IEEE 754 double-precision Sparse Matrix-Vector Multiplication (\texttt{SpMV}): $y = \alpha\cdot\text{op}\left(A\right)\cdot x + \beta\cdot y $}
  ...
  \begin{tabularx}{\textwidth}{l l X c}
    ...
    \textbf{GraphBLAS Suite Sparse} & C & \texttt{GrB\_mxm(...)} & Direct \\
    \textbf{ALP/GraphBLAS} & C++ & \texttt{grb::mxv<op>(y, A, x, semiring)} & Non-blocking \\
    \textbf{cuSPARSE} & CUDA C & \texttt{cusparseSpMV(handle, ...)} & Multi-stage \\
    \textbf{rocSPARSE} & ROCm C & \texttt{rocsparse\_spmv(handle, ...)} & Multi-stage \\
    \textbf{hipSPARSE} & C++ & \texttt{hipsparseDcsrmv(handle, ...)} & Legacy direct \\
    \textbf{oneMKL (SYCL)} & SYCL & \texttt{oneapi::mkl::sparse::gemv(queue, ..., \{ev\_opt\})} & Out-of-order + Opt \\
    \textbf{oneMKL (SYCL)} & SYCL & \texttt{oneapi::mkl::sparse::gemv(queue, ...)} & In-order + Opt \\
...
For sparse formats that are row oriented, like CSR, the SpMV routine is often trivial to parallelize by partitioning the input matrix rows among different execution units.  For other sparse formats like COO or CSC, more sophisticated parallelization schemes are required, either requiring large temporary workspaces for execution units, or atomic access to the output object to avoid write conflicts. Applications, for example those that result in matrices with high skews in row-degrees, may also require additional workspace for intra-row parallelization even when using standard CSR. Nevertheless, the straightforward exposure of parallelism and the option of expressing the kernel as single stage routine characterizes kernels that involve at least one sparse matrix and a known output shape/size object. Examples are
\begin{itemize}
...
\item Frobenius norm ( sparse matrix -> scalar ), and
...
For the SpGEMM routine, different parallelization strategies can be realized, ranging from row-parallel and column-parallel to sub-matrix parallel and element-parallel, but all coming with their strategy-specific downsides. Functionality falling into the group represented by the SpGEMM include
\begin{itemize}
...
The following sections discuss ownership of the input and output data structures as well as optimization in more detail,
but already here we mention that the sparse input matrices could be enhanced with optimization-enabling information while sparse output matrices are expected to hold no such information.
...
This non-owning approach limits the optimization potential for the libraries.
To nevertheless allow for optimization while,
to a large degree,
preserving the concept of light-weight views, we encapsulate the \texttt{xyz\_wrapper} in a \texttt{matrix\_opt} that contains both the wrapper around the user-owned raw data and a structure that can contain additional data for optimization. All the optimization data is owned by the library, not by the user, and the data is stored in library-allocated memory that is opaque to the user.

Some users want to keep control of all available memory, in particular, control where data structures are allocated by the library. For this reason, we propose the user passes an \texttt{allocator} to the library that the library uses to allocate additional data in the \texttt{matrix\_opt}. If no allocator is provided, the library is permitted to use whichever allocator they deem to be appropriate.


Listing~\ref{fig:csrmatrixhandle} demonstrates how to wrap a user-owned CSR matrix structure, then encapsulate the ``format wrapper'' into a \texttt{matrix\_opt}. 
...
% \newpage
\input{graphics/fig_spgemm_workflows}
% \newpage
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{X >{\centering\arraybackslash}p{3.8cm} c c}
    ...
    Sparse Matrix -- Sparse Matrix Addition & $C := A + B$ & SpGEMM-type & Multi-stage \\
    Sparse Matrix -- Sparse Matrix Multiplication & $C := A \cdot B + D$ & SpGEMM-type & Multi-stage \\
    Sparse Matrix -- Dense Matrix Multiplication & $Y := \alpha A \cdot X + \beta Z$ & SpMV-type & Single-stage \\
    Sparse Matrix -- Dense Vector Multiplication & $y := \alpha A \cdot x + \beta z$ & SpMV-type & Single-stage \\
    Vector Triangular Solve & Solve $A \cdot y = x$ for $y$ & SpTRSV-type & Single-stage \\
    Matrix Triangular Solve & Solve $A \cdot Y = X$ for $Y$ & SpTRSM-type & Single-stage \\
    Sparse Matrix Format Conversion & $B = \text{sparse}(A)$ & SpGEMM-type & Multi-stage \\
    Predicate Selection & $B = A(\text{predicate})$ & SpGEMM-type & Multi-stage \\
    Sampled Dense -- Dense Matrix Multiplication & $C\langle\text{mask}\rangle = A \cdot B$ & SpMV-type & Single-stage \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{acks}
UT-Battelle LLC has authored this manuscript under contract no. DE-AC05-00OR22725 with the US Department of Energy. By accepting the article for publication, the publisher acknowledges that the US government retains a non-exclusive, paid-up, irrevocable, worldwide license to publish or reproduce the published form of the manuscript or allow others to do so, for US government purposes. The DOE will provide public access to these results in accordance with the DOE Public Access Plan.
\end{acks}
