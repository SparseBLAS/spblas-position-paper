...
\section{The Three Archetypes for the Sparse BLAS Design}
\label{sec:archetype}
While there exists a long list of sparse linear algebra functionality, for the design of an interface standard, it is useful to identify groups of functionality sharing the same properties. We identified three distinct archetypes based on their computational complexity, memory requirements, and parallelization strategies. 

The first archetype is the \textbf{Sparse Matrix-Vector Product} (SpMV), which is defined by the multiplication of a sparse matrix by a dense vector to produce a dense output vector. It is generally considered the most predictable operation because the output size is known beforehand, allowing for straightforward memory allocation. While the design of an interface should support various input formats (e.g., by accepting an abstract matrix operator), the operation is typically memory-bandwidth limited and trivial to parallelize by partitioning the matrix rows among different execution units. 


In contrast, \textbf{Sparse Matrix-Matrix Multiplication} (SpGEMM) represents a much more complex archetype due to the unknown output sparsity pattern. Because the number of non-zero entries in the resulting matrix cannot be determined a priori, a multi-stage design is needed. This process begins with an inspection or symbolic phase to compute the sparsity pattern, followed by memory allocation, and finally a numeric phase to calculate the actual values. Because the workload can be highly unbalanced, the parallelization strategies for SpGEMM can range from row-parallel and column-parallel to sub-matrix parallel and element-parallel.

The third archetype, the \textbf{Sparse Triangular Solve} (SpTRSV), involves solving systems where the sparse matrix is triangular or only a triangular part of the matrix is considered. For this reason, the design relies heavily on property tags to inform the solver whether the matrix stored is lower or upper triangular or if it possesses symmetry that can be exploited for faster execution. Although the output size is as predictable as that of SpMV, the operation is inherently sequential because each element of the output vector may depend on previously computed values. To extract parallelism, the library must often construct a dependency graph or utilize level-scheduling to identify which components can be computed simultaneously, making performance highly sensitive to the specific sparsity pattern of the matrix. 
