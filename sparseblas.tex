...
%% command.
\PassOptionsToPackage{dvipsnames,svgnames,x11names}{xcolor} % Kept coherence with original document but goes against acm rules (line above)
...
% Required utilities
\usepackage{xspace} % Used for the \cplusplus command
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{array}
...
% I THINK THAT THESE PACKAGES ARE NOT NEEDED ANYMORE
% \usepackage{authblk} % REMOVE
% \usepackage{mathptmx} % REMOVE (font set from acm)
% \usepackage[scaled=.89]{helvet} % REMOVE (font set from acm)
% \usepackage[dvipsnames,svgnames,x11names]{xcolor} % REMOVE: Pass options to acmart defiend xcolor
% \usepackage[svgnames]{xcolor}
% \usepackage{comment}
% \usepackage{placeins} % REMOVE? Forcing float placement, maybe not good for ACM template
% \usepackage[colorlinks,linkcolor=MediumSlateBlue,citecolor=MediumSlateBlue,urlcolor=MediumSlateBlue]{hyperref} % REMOVE (acmart defines it internally)
% \usepackage{cleveref}
% \usepackage{marginnote}
% \usepackage[T1]{fontenc} % REMOVE (font set from acm)
% \usepackage{multicol}
...
\author{Hartwig Anzt}
\authornote{Coordinator of the SparseBLAS effort.}
\affiliation{%
...
\author{Sherry Li}
\affiliation{%
  \institution{Lawrence Berkeley National Labs}
  \city{Berkeley}
  \country{USA}
}
...
\authornote{Research was sponsored by the Department of the Air Force Artificial Intelligence Accelerator and was accomplished under Cooperative Agreement Number FA8750-19-2-1000. The views and conclusions contained in this document are those of the authors and should not be interpreted as representing the official policies, either expressed or implied, of the Department of the Air Force or the U.S.\ Government. The U.S.\ Government is authorized to reproduce and distribute reprints for Government purposes notwithstanding any copyright notation herein.}
\affiliation{%
  \institution{University of Tennessee}
  \city{Knoxville}
  \country{USA}
...
\renewcommand{\shortauthors}{Anzt et al.}
...
yang2022graphblast},
targeting nonblocking execution, distributed systems, and/or GPU accelerators.
The GraphBLAS \cplusplus Specification provides
...
overhead.  This issue can be resolved with the load/unload methods in
SuiteSparse:GraphBLAS, which takes $O(1)$ time and does not allocate memory. 
ALP/GraphBLAS provides similar wrappers and avoids unload through standard C++ features.
...
and on implementation of the library using
multiple layers. Base functionality is implemented in Fortran 77 and
...
While none of the SparseBLAS standards discussed in~\ref{sec:existingsparseblas} has been adopted as a universal standard, several groups from academia and industry have developed and deployed sparse linear algebra operations along with a user-facing interface. They are all designed to accommodate functionality- and hardware-specific needs of the library developers. Most of the currently supported APIs follow the transparent, non-data-owning design. 
...
\begin{table*}[t]
  \centering
  \caption{Comparison of SpMV execution APIs across vendor and domain libraries for $y = \alpha \cdot \text{op}(A) \cdot x + \beta \cdot y$.}
  \label{tab:spmv_api_comparison}
  \small
  \begin{tabularx}{\textwidth}{l l X c}
    \toprule
    \textbf{Library} & \textbf{Paradigm} & \textbf{Key Function / Entry Point} & \textbf{Stage Model} \\
    \midrule
    \textbf{ArmPL} & C & \texttt{armpl\_spmv\_exec\_d(...)} & Hint / Opt \\
    \textbf{oneMKL (C)} & C & \texttt{mkl\_sparse\_d\_mv(...)} & Hint / Opt \\
    \textbf{AOCL-Sparse} & C & \texttt{aoclsparse\_dmv(...)} & Hint / Opt \\
    \textbf{GraphBLAS Suite} & C & \texttt{GrB\_mxm(...)} & Direct \\
    \textbf{ALP/GraphBLAS} & C++ & \texttt{grb::mxv<op>(y, A, x, semiring)} & Non-blocking \\
    \textbf{cuSPARSE} & CUDA C & \texttt{cusparseSpMV(...)} & Multi-stage \\
    \textbf{rocSPARSE} & ROCm C & \texttt{rocsparse\_spmv(...)} & Multi-stage \\
    \textbf{hipSPARSE} & C++ & \texttt{hipsparseDcsrmv(...)} & Legacy direct \\
    \textbf{oneMKL (SYCL)} & SYCL & \texttt{oneapi::mkl::sparse::gemv(queue, ...)} & Out-of-order / Opt \\
    \textbf{Kokkos-Kernels} & C++ & \texttt{KokkosSparse::spmv(controls, ...)} & Direct \\
    \textbf{Ginkgo} & C++ & \texttt{A->apply(alpha, x, beta, y)} & Executor-bound \\
    \bottomrule
  \end{tabularx}
\end{table*}

% Re-factored into single listing of option 1.
\begin{listing}[htbp]
\caption{\texttt{SpMV} API comparison ($y = \alpha \cdot \text{op}(A) \cdot x + \beta \cdot y$) across libraries.}
...
// (4) GraphBLAS Suite Sparse C API (y<mask) = accum(y, A*x))
GrB_apply(y, GrB_NULL, GrB_NULL, GrB_TIMES_FP64, alpha, y, GrB_NULL);
GrB_mxm(y, GrB_NULL, GrB_PLUS_FP64, GrB_PLUS_TIMES_SEMIRING_FP64, A, x, GrB_NULL);

// (5) ALP/GraphBLAS C++ (Optimised by nonblocking execution)
status = grb::mxv<op>(y, A, x, plusTimes);

// (6) cuSPARSE CUDA C (Multi-stage generic API)
...
// (7) rocSPARSE ROCm C (Multi-stage generic API)
...
// (9) oneMKL SYCL C++ API (Out-of-order queue)
ev_gemv = oneapi::mkl::sparse::gemv(queue, oneapi::mkl::transpose::nontrans, 
                                    alpha, csrA, x, beta, y, {ev_opt});

// (10) oneMKL SYCL C++ API (In-order queue)
ev_gemv = oneapi::mkl::sparse::gemv(queue, oneapi::mkl::transpose::nontrans, 
                                    alpha, csrA, x, beta, y);
...
% I changed the lstlisting to minted so that we can have only the mitned package and not the listing one on top.
% \begin{lstlisting}
% // comm
% /* comm */
% info = armpl_spmv_exec_d(ARMPL_SPARSE_OPERATION_NOTRANS, alpha, armpl_mat, x, beta, y);
...
The functionality we consider in scope of this effort is listed in~\ref{sec:functionality}. Instead of discussing the characteristics of the distinct SparseBLAS routines individually, we identify in~\ref{sec:archetype} a set of archetypes that represent functionality groups. Before that, the concept and standardization of sparse storage formats is addressed in~\ref{sec:sparse-formats}. In~\ref{sec:ownership}, we discuss the opaqueness and ownership of the sparse objects, and how a combination of data wrappers and objects can be used to find a balance between accessibility and performance optimization aspects.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{l l X}
    ...
    --- & \texttt{T} & Type of values stored in \texttt{mdspan}, equivalent to \texttt{scalar\_type}. \\
    --- & \texttt{Extents} & Type specifying the number of dimensions, their types, and whether they are known at compile time or runtime. \\
    --- & \texttt{LayoutPolicy} & A type defining how matrix elements are laid out (e.g., \texttt{std::layout\_left}, \texttt{std::layout\_right}). \\
    --- & \texttt{AccessorPolicy} & A type defining how matrix elements should be accessed. \\
    \addlinespace
    \texttt{data\_handle\_type} & \texttt{p} & Handle to dense matrix values. \\
    \texttt{std::integral} type & \texttt{exts...} & The matrix's extents, or dimensions. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
%\begin{itemize}
%\item 
With \textbf{fully owning} APIs, the library allocates and owns the data; while
%\item 
\textbf{non-owning} APIs assume the user allocates and owns the data.
%\end{itemize}
A separate axis separates APIs that expose \textbf{fully transparent} data structures frome those with \textbf{opaque} data structures.

The GraphBLAS C Specification exposes a fully owning, opaque API which some implementations extended to handle a mixture of fully owned and non-owned data.
In contrast,
BLAS and LAPACK expose non-owning APIs with transparent data types.
For sparse BLAS,
multiple reasons for either type of API exist:
1) non-owning matrix objects can prevent some vendor optimizations (e.g., using a better-suited matrix format), while owning matrix objects come with an initial usage cost and may require copies;
2) non-owning matrix objects may require multiple phases for their instantiation and allocation that must be explicitly exposed to the user and interleaved with library calls, while owning matrix objects can prevent the user-code complexity this entails;
3) opaque objects require accessors that can come with an extra cost while transparent objects enable users to inspect and access data at little cost.

Because each of these aspects can be important in different situations and to different users, we believe that mixtures of these choices are both needed as well as complementary, satisfying most needs. Generally, designing a non-owning, transparent API for functionality is significantly harder. For this reason, this first version of an interface design focuses on this option first:
%As we will detail next, we extend this choice to allow for a moderate level of optimization
%by mixing both (non-)owning and transparent/opaque APIs:
%AJ, note: I disabled the above as it comes back more clearly later. Adding it here would expose that the current proposed API is a mixture of (non-)owning and opaque/transparent, while it serves the position paper better if it communicates a clear overarching choice first
the user always owns the data while a light-weight construct exposes the data structures to the library operations, which the \cplusplus views concept standardises~\cite{cpplinalg:p1673r13}-- as we will discuss next.
...
In Listing~\ref{fig:csrmatrixhandle} we demonstrate how to wrap a user-owned CSR matrix structure, then encapsulate the ``format wrapper'' into a \texttt{matrix\_opt}. 
...
\begin{enumerate}
    ...
    \item Generally speaking, all SparseBLAS functionality will take \texttt{matrix\_opt} objects as the matrix input arguments and if a SparseBLAS routine creates a sparse matrix as an output, the output argument shall be an \texttt{xyz\_wrapper}, as it is created and filled over time through the distinct stages of the functionality.
    \item A SparseBLAS function shall only modify user-owned data, in a \texttt{matrix\_opt} or an \texttt{xyz\_wrapper}, if it is explicitly detailed in the functionality description that it will do so, like to sort the matrix or update provided data for the matrix, or to fill in resulting matrix data from a copy or transpose or other operation creating a new matrix.
    \item Libraries or users are allowed to modify the properties (sorted, symmetric pattern, etc) in an \texttt{xyz\_wrapper} at any time as said property is discovered to be true or known to be true based on a particular algorithm whether the \texttt{xyz\_wrapper} is on its own or in a \texttt{matrix\_opt}.
\end{enumerate}
...
Already in the archetype categorization, we noticed two fundamentally different operation classes:
\begin{enumerate}
...
can be realized using the three-stage API sequence shown in Listing~\ref{lst:spgemm_threestage}.
...
\begin{listing}[htbp]
\caption{Three-stage API sequence for explicit symbolic/numeric separation in \texttt{SpGEMM}.}
\label{lst:spgemm_threestage}
\begin{minted}[fontsize=\small]{c++}
using namespace spblas;
// csr_wrapper A, B, D initialized elsewhere
// csr_wrapper C(c_nrows, c_ncols);
// auto A_obj = matrix_opt(A, allocator);
// auto B_obj = matrix_opt(B, allocator);
// auto D_obj = matrix_opt(D, allocator);
...
// Optional optimization / hint step
multiply_inspect(policy, state, alpha, A_obj, B_obj, beta, D_obj, C);

// Computes output sparsity pattern and memory requirements
multiply_compute_symbolic(policy, state, alpha, A_obj, B_obj, beta, D_obj, C);

index_t nnz = state.get_result_nnz();
// User allocates the required storage arrays for C here

// Populates the structural non-zero sparsity pattern into C
multiply_fill_symbolic(policy, state, alpha, A_obj, B_obj, beta, D_obj, C);

// Computes and stores the numerical values into C
multiply_numeric(policy, state, alpha, A_obj, B_obj, beta, D_obj, C);
// csr_wrapper C is now fully constructed
\end{minted}
\end{listing}
...
the simplified two-stage SpGEMM interface shown in Listing~\ref{lst:spgemm_twostage} can be used for Equation~\eqref{eq:SpGEMM}.
...
\begin{listing}[htbp]
  \caption{Alternative two-stage API for \texttt{SpGEMM} combining symbolic and numeric resolution.}
  \label{lst:spgemm_twostage}
  \begin{minted}[fontsize=\small]{c++}
using namespace spblas;
// csr_wrapper<float> A, B, D initialized elsewhere
// csr_wrapper<float> C(c_nrows, c_ncols);
// auto A_obj = matrix_opt(A, allocator);
// auto B_obj = matrix_opt(B, allocator);
// auto D_obj = matrix_opt(D, allocator);
...
// Computes output sparsity structure and required memory size
multiply_compute(policy, state, alpha, A_obj, B_obj, beta, D_obj, C);

index_t nnz = state.get_result_nnz();
// User allocates the required storage arrays for C here

// Populates structural indices and numerical values into C
multiply_fill(policy, state, alpha, A_obj, B_obj, beta, D_obj, C);
// csr_wrapper C is now fully constructed
  \end{minted}
\end{listing}

Also this interface gives some freedom on how implementations arrange the different computational steps in the distinct stages. Instead of discussing the different possibilities, we visualize in \ref{fig:SpGEMM_workflows} different implementation possibilities for both the three-stage SpGEMM API and the two-stage SpGEMM API.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{l X}
    \toprule
    \textbf{\texttt{view}} & \textbf{Description} \\
    \midrule
    \texttt{spblas::view::transpose(M)}           & Expose transposed data of argument \texttt{M}. \\
    \texttt{spblas::view::conjugate(M)}           & Expose conjugated data of argument \texttt{M}. \\
    \texttt{spblas::view::triangle(M, uplo, diag)}  & Expose \texttt{uplo} triangular data of argument \texttt{M}. \\
    \texttt{spblas::view::diagonal(M, diag)}      & Expose diagonal data (not strictly lower or upper triangle) of argument \texttt{M}. \\
    \texttt{spblas::view::symmetric(M, uplo, diag)} & Expose symmetric matrix using the \texttt{uplo} triangular data of argument \texttt{M}. \\
    \texttt{spblas::view::hermitian(M, uplo, diag)} & Expose Hermitian matrix using the \texttt{uplo} triangular data of argument \texttt{M}. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\subsection{Functionality Scope of the SparseBLAS}
...
%     \textbf{Type} & \textbf{Definition} & \textbf{Notes} \\
%     \midrule
%     \texttt{csr\_wrapper<T>} 
%       & Non-owning view class for CSR sparse matrices. Similar types will exist for other formats in the standard. 
%       & Transparent type. Must behave like a C++ view: lazy evaluation, constant copy time, etc.\footnote{\url{https://en.cppreference.com/w/cpp/ranges/view}} Operations take either views or matrix handles. \\
%     \addlinespace
%     \texttt{matrix\_opt} 
%       & A type which encapsulates a matrix view as well as opaque library data specific to the matrix. 
%       & The opaque part should be relevant to the matrix itself, rather than any particular operation. Typically, the opaque part contains stored optimization information. Operations typically take \texttt{matrix\_opt} as input arguments and \texttt{format\_wrapper} as output arguments. \\
%     \addlinespace
%     \texttt{operation\_state} 
%       & Object storing state associated with a SparseBLAS algorithm. Has one member function, \texttt{get\_result\_nnz()}, which returns the number of nonzeros in the output of the operation. 
%       & Opaque type: implementation defined. Typically contains stored optimization information. Optional first (if no \texttt{policy} is provided) or second argument to all operation API calls. \\
%     \addlinespace
%     \texttt{allocator} 
%       & User-provided allocator function to be used for memory allocations by \texttt{operation\_state\_t} and \texttt{matrix\_opt} objects. 
%       & \texttt{operation\_state\_t} and \texttt{matrix\_opt} will provide a constructor which takes an allocator, as well as a default (in which case the allocator is implementation-defined). \\
%     \addlinespace
%     \texttt{policy} 
%       & C++ \texttt{std::execution} policy extended with vendor specific policies. 
%       & Opaque type. Implementations are free to interpret policies as they wish and extend the set of policies supported. Optional first argument to all operation API calls. \\
%     \bottomrule
%   \end{tabularx}
%   \caption{Summary of types introduced by the standard.} 
%   \label{tab:newtypes}
...
While the most fundamental part of the standardization effort is to design an interface for the three archetypes, we anticipate a comprehensive SparseBLAS implementation to include additional routines. Particularly, we understand the SparseBLAS effort to entail \textit{conversion between sparse formats} listed in~\ref{sec:sparse-formats}, the \textit{multiplication and addition of matrices} (including operations involving both sparse and dense matrices), \textit{transposition and conjugation} of sparse matrices, \textit{a solver for sparse triangular linear systems}, \textit{scaling} of sparse matrices, \textit{norm} calculations on sparse matrices, \textit{element-wise addition and multiplication}, and \textit{masking operations} that extract a subset of the nonzero entries of a sparse matrix as in the sampled dense times dense matrix multiplication (SDDMM) kernel common in machine learning frameworks. A list of functionality falling into this scope is given in \ref{tab:scope}. For convenience, we also summarize the API type for each functionality as detailed in \ref{sec:api-design}. Multi-stage APIs produce a sparse matrix as an output with a sparsity pattern unknown prior to the computation, whereas single-stage APIs produce a dense output or a sparse output with a sparsity pattern beforehand.
...
%        Vector Triangular Solve                                   & Solve $A \cdot y = x$ for $y$  & SpTRSV-type & Single-stage \\
%        Matrix Triangular Solve                                   & Solve $A \cdot Y = X$ for $YA$  & SpTRSM-type & Single-stage \\
...
\begin{table}[htbp]
  ...
  \caption{List of the SparseBLAS functionalities. Uppercase letters represent matrices; lowercase letters represent vectors. Letters from the start of the alphabet are used for sparse matrices/vectors; letters from the end are used for dense matrices/vectors. $\alpha$ is a scalar value. Note that scaling may be applied to any input matrix/vector, and the transpose/conjugate transpose operation may be applied to any input matrix.}
  \label{tab:scope}
  \begin{tabularx}{\linewidth}{X >{\centering\arraybackslash}p{3.8cm} c c}
    \toprule
    \textbf{Operation} & \textbf{Notation} & \textbf{Archetype} & \textbf{API Type} \\
    \midrule
    Scaling & $A := \alpha A$ & SpMV-type & Single-stage \\
    ...
    Infinity Matrix Norm & $\alpha := \|A\|_\text{inf}$ & SpMV-type & Single-stage \\
    Frobenius Matrix Norm & $\alpha := \|A\|_F$ & SpMV-type & Single-stage \\
    Element-wise Multiplication & $C := A \cdot* B$ & SpMM-type & Multi-stage \\
    Sparse Matrix -- Sparse Matrix Addition & $C := A + B$ & SpMM-type & Multi-stage \\
    Sparse Matrix -- Sparse Matrix Multiplication & $C := A \cdot B + D$ & SpMM-type & Multi-stage \\
    Sparse Matrix -- Dense Matrix Multiplication & $Y := \alpha A \cdot X + \beta Z$ & SpMV-type & Single-stage \\
    Sparse Matrix -- Dense Vector Multiplication & $y := \alpha A \cdot x + \beta z$ & SpMV-type & Single-stage \\
    Vector Triangular Solve & Solve $A \cdot y = x$ for $y$ & SpTRSV-type & Single-stage \\
    Matrix Triangular Solve & Solve $A \cdot Y = X$ for $Y$ & SpTRSM-type & Single-stage \\
    Sparse Matrix Format Conversion & $B = \text{sparse}(A)$ & SpMM-type & Multi-stage \\
    Predicate Selection & $B = A(\text{predicate})$ & SpMM-type & Multi-stage \\
    Sampled Dense -- Dense Matrix Multiplication & $C\langle\text{mask}\rangle = A \cdot B$ & SpMV-type & Single-stage \\
    \bottomrule
