...
By adopting specialized data structures that omit the majority of these zeros, we unlock significant savings in memory footprint, memory access volume, and overall computational cost. Rather than wasting cycles and bandwidth on redundant operations—such as multiplying by zero or loading null values from RAM—sparse routines focus exclusively on the non-zero elements. While these formats introduce greater complexity in how we index and access data, the resulting gains in performance and scalability are indispensable. This necessity motivates the entire ecosystem of sparse data structures and the specialized routines designed to navigate them, transforming what would be computationally impossible into a manageable task.
...
In contrast to the SpMV kernel, the \textbf{Sparse Matrix-Matrix Multiplication} (SpGEMM) forming the product of two sparse matrices represents a class of much more complex routines that share the property of generating a sparse matrix as output with unknown sparsity pattern. In consequence, the sparse data structure for the output can not be allocated prior to the invocation of the functionality. The functionality can only be implemented as multi-stage routine. This process begins with the first stage computing the size of the output, the allocation of the output data structure, and the final stage filling the output data structure. In~\Cref{sec:ownership} we will discuss the ownership of the input and output data structures, but already here we mention that the sparse input matrices could be enhanced with optimization-enabling information, the sparse output matrix is expected to be a plain sparse matrix. For the SpGEMM routine, different parallelization strategies can be realized, ranging from row-parallel and column-parallel to sub-matrix parallel and element-parallel, but all coming with their strategy-specific downsides.

The third archetype, the \textbf{Sparse Triangular Solve} (SpTRSV), solves a linear system $Ax=b$ with sparse triangular matrix $A$. The sparse triangular matrix $A$ may be stored as a triangular matrix, or as a full matrix, and the SpTRSV is required to extract the triangular part during kernel execution. This, oppose to the other archetypes, requires the SpTRSV to heavily rely on auxiliary information stroed in the property tags we discuss in~\Cref{sec:views}.  Like for the SpMV routine, the size of the output vector is known a priori, and the routine can be realized in a single kernel. On the other hand, conversely to the SpMV, the parallelization of the SpTRSV kernel is significantly more challenging, as the operation is inherently sequential because each element of the output vector may depend on previously computed values. To extract parallelism, the implementations typically construct a dependency graph or utilize level-scheduling to identify which components can be computed simultaneously, making performance highly sensitive to the specific sparsity pattern of the matrix. In~\Cref{sec:sparseblasimpl} we discuss the addition of an ``inspect'' routine that accompanies all sparse BLAS routines and can carry the load of determining a dependency graph for a sparse triangular solve.
...
\begin{table}[h]
    ...
    \midrule
       ...
       Sparse Matrix -- Dense Matrix Multiplication       & $Y := \alpha A \cdot X + \beta Z$  & Single-stage \\
       Sparse Matrix -- Dense Vector Multiplication       & $y := \alpha A \cdot x + \beta z$  & Single-stage \\
