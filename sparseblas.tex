...
In \Cref{fig:csrmatrixhandle} we demonstrate how the ``matrix\_wrapper'' wraps a user-owned CSR matrix structure, then encapsulates the ``matrix\_wrapper'' into a \texttt{matrix\_opt}. 


Generally speaking, any sparse BLAS functionality should take \texttt{matrix\_opt}s as the input arguments and create matrix\_wrappers as output arguments - if creating sparse matrices as output.
\todo{How about a masking operation? Is the input mask a  \texttt{matrix\_opt} or a \texttt{matrix\_opt}? Where is the output stored in?}
Furthermore, any change to a matrix\_wrapper that is wrapped into a  \texttt{matrix\_opt} has to be propagated explicitly to the \texttt{matrix\_opt}, as any library-owned optimization would otherwise not know about the changes in the matrix\_wrapper. Conversely, only a few sparse BLAS functionalities change the matrix\_wrapper underlying a \texttt{matrix\_opt} input argument. (These include sorting, transposing, etc.) 
...
For situations where a sparse object is not used in its original form, but in a modification, e.g., only a submatrix (lower triangular) or in transposed form, modifying the user-owned data in the matrix\_wrapper conflicts with the non-mutability of the user-owned data, and creating an explicit copy can introduce significant overhead. We resolve the situation by introducing the \texttt{view} instances shown in Table~\ref{tab:views}. Note that \verb|uplo| is one of \texttt{view::uplo::\{lower, upper\}}, and \texttt{diag} is one of \texttt{view::diag::\{explicit, implicit\_unit, implicit\_zero\}}.
