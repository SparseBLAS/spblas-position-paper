...
\author[1,2]{Hartwig Anzt\footnote{Coordinator of the Sparse BLAS effort.}}
...
Nevertheless, we anticipate that the proposed standard and GraphBLAS will overlap for a subset of functionalities, and the Sparse BLAS API will be capable of efficiently expressing them. GraphBLAS innovations we adopt in this iteration of the Sparse BLAS standard include: (1) operations between two sparse objects, (2) masking, and (3) non-blocking or asynchronous execution.
...
While none of the sparse BLAS standards discussed in~\Cref{sec:existingsparseblas} has been adopted as a universal standard, several groups from academia and industry have developed and deployed sparse linear algebra operations along with a user-facing interface. They are all designed to accommodate functionality- and hardware-specific needs of the library developers. Most of the currently supported APIs follow the transparent, non-data-owning design. 
...
This requires to consider a wide range of design aspects, including storage formats, ownership and opaqueness of objects, functionality scope, multi-stage functions execution policies. 
...
%Even so, the Sparse BLAS serves as an important foundation for building high-level applications, facilitating integration between advanced algorithms and low-level building blocks.
The functionality we consider in scope of this effort is listed in~\Cref{sec:functionality}. Instead of discussing the characteristics of the distinct sparse BLAS routines individually, we identify in~\Cref{sec:archetype} a set of archetypes that represent functionality groups. Before that, the concept and standardization of sparse storage formats is addressed in~\Cref{sec:sparse-formats}. In~\Cref{sec:ownership}, we discuss the opaqueness and ownership of the sparse objects, and how a combination of data wrappers and objects can be used to find a balance between accessibility and performance optimization aspects.
...
The fundamental distinction between dense and sparse linear algebra lies in the strategic move away from contiguous data storage toward more resource-efficient formats. In dense linear algebra, matrices are represented as exhaustive arrays where every entry occupies a memory slot. If sparse matrices were stored in this same manner, the need for a specialized sparse BLAS API would vanish, as the functionality would be identical to existing dense routines. However, the defining characteristic of a sparse matrix—the overwhelming prevalence of zero entries—renders such an approach prohibitively expensive for large-scale applications.
...
\subsection{The Three Archetypes for the Sparse BLAS Design}
\label{sec:archetype}
...
In~\Cref{sec:sparseblasimpl}, we discuss the addition of an ``inspect'' routine that accompanies all sparse BLAS routines and can carry the load of determining a dependency graph for a sparse triangular solve.
...
\begin{table}[h]
    ...
    %\bottomrule
    \end{tabular}
    \caption{Instances of the \texttt{view} wrapper in Sparse BLAS.} add 
    \label{tab:views}
\end{table}
...
Any interface and functionality standardization must allow for horizontal and vertical interoperability. In the context of the sparse BLAS API,
