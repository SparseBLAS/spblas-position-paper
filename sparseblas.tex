...
Nevertheless, we anticipate that the proposed standard and GraphBLAS will overlap for a subset of functionalities, and the SparseBLAS API will be capable of efficiently expressing them. GraphBLAS innovations we adopt in this iteration of the SparseBLAS standard include: (1) operations between two sparse objects, (2) masking, and (3) non-blocking or asynchronous execution. 
...
\newpage
\noindent\textbf{\texttt{SpMV} API comparisons between existing libraries:}\\ IEEE754 Double Precision Sparse Matrix-Vector Multiplication (\texttt{SpMV}) \begin{equation*}y = \alpha\cdot\text{op}\left(A\right) x + \beta\cdot y\end{equation*}
...
To reduce complexity, we have omitted advanced functionalities that operate on sparse objects such as iterative solvers, direct solvers, eigensolvers, and preconditioners. 
...
\subsection{Four Archetypes for the SparseBLAS Design}


\begin{table}[htbp]
\centering
\caption{2D Classification Grid for Sparse Matrix Operations based on Output Memory Management and Parallel Dependencies.}
\label{tab:sparse_2d_classification}
\vspace{0.5em}

% Apply smaller base font size and optimized padding
\footnotesize
...
\toprule
& \textbf{Parallelism Known \textit{a priori}} \newline \scriptsize(Independent Work Units)
& \textbf{Parallelism Pattern-Dependent} \newline \scriptsize(Requires Runtime Dependency Graph) \\
\midrule

\textbf{Output Sparsity Known} \newline \scriptsize(Single-Stage API)
& \textbf{SpMV / SpMM / Scaling} \newline
  -- Output sparsity pattern known upfront. \newline
  -- Rows/blocks can be processed independently without cross-thread dependencies.
& \textbf{SpTRSV} \textit{(Sparse Triangular Solve)} \newline
  -- Output vector size known upfront. \newline
  -- Thread execution strictly ordered by dependency DAG / level-sets. \\

\addlinespace[0.6em]

\textbf{Output Sparsity Unknown} \newline \scriptsize(Multi-Stage API)
& \textbf{SpGEMM / SpADD} \newline
  -- Output sparsity pattern unknown upfront. \newline
  -- Independent output rows (embarrassingly parallel; symbolic phase used for memory allocation).
& \textbf{Sparse Factorizations} \textit{(LU, Cholesky, QR)} \newline
  -- Output fill-in structure unknown upfront. \newline
  -- Execution ordered by elimination tree / DAG (symbolic phase used for scheduling \& allocation). \\
\bottomrule
\end{tabularx}
\end{table}


To systematically organize the landscape of sparse matrix operations, algorithms can be classified along two orthogonal architectural axes: \textit{output sparsity predictability} (which dictates memory allocation and API staging) and \textit{data-dependency structure} (which governs parallel thread scheduling). This taxonomy gives rise to four primary archetypes that capture the distinct execution paradigms of sparse linear algebra:

\begin{itemize}
    \item {SpMV Archetype} (\textit{Known Sparsity, Static Parallelism}): Represents operations like Sparse Matrix-Vector multiplication, where output sizes are pre-allocated and threads process independent rows without data dependencies.
    \item {SpTRSV Archetype} (\textit{Known Sparsity, Pattern-Dependent Parallelism}): Represents Sparse Triangular Solves, where the output vector size is known upfront, but thread execution requires level-set scheduling based on runtime dependency graphs.
    \item {SpGEMM Archetype} (\textit{Unknown Sparsity, Static Parallelism}): Represents Sparse Matrix-Matrix multiplication, where output memory must be determined mid-flight via symbolic inspection, yet individual output rows remain embarrassingly parallel.
    \item {Sparse Factorizations Archetype} (\textit{Unknown Sparsity, Pattern-Dependent Parallelism}): Represents direct solvers (LU, Cholesky, QR), which require symbolic phases for both dynamic memory allocation and graph-based elimination trees to resolve complex execution dependencies.
\end{itemize}

A visualization of this categorization is available in Table~\ref{tab:sparse_2d_classification}, where we map the sparse routines along these two dimensions to create operational taxonomy. Operations residing in the upper row (\textit{Output Sparsity Known}) can be executed using simple single-stage APIs, whereas those in the lower row (\textit{Output Sparsity Unknown}) necessitate multi-stage workflows to handle dynamic memory allocation. The columns delineate the thread-scheduling overhead: operations on the left feature statically known, embarrassingly parallel work units, while those on the right demand runtime graph inspection (such as level-set or elimination tree analysis) to resolve data dependencies before safe parallel execution can begin.
...
Aside from the archetype categorization, an orthogonal classification distinguishes between: 
\begin{enumerate}
...
In this first edition, we propose a non-owning non-opaque API. As discussed, an opaque owning API allows the libraries to perform additional optimization, such as lazy evaluation of routines. We postpone the design of such an API to the future. 
