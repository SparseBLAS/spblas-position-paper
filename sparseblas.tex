...
In \Cref{fig:csrmatrixhandle} we demonstrate how the ``format\_wrapper'' wraps a user-owned CSR matrix structure, then encapsulates the ``format\_wrapper'' into a \texttt{matrix\_opt}. 


Generally speaking, any sparse BLAS functionality should take \texttt{matrix\_opt}s as the input arguments and create format\_wrappers as output arguments - if creating sparse matrices as output.
...
Furthermore, any change to a format\_wrapper that is wrapped into a  \texttt{matrix\_opt} has to be propagated explicitly to the \texttt{matrix\_opt}, as any library-owned optimization would otherwise not know about the changes in the format\_wrapper. Conversely, only a few sparse BLAS functionalities change the format\_wrapper underlying a \texttt{matrix\_opt} input argument. (These include sorting, transposing, etc.) 
...
\begin{table}[h]
    ...
    %\toprule
    \hline
    \makecell[c]{\textbf{\texttt{view}}} & \makecell[c]{\textbchaf{Description}}\\
    %\midrule
    \hline
