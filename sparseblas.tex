...
The first archetype is the \textbf{Sparse Matrix-Vector Product} (SpMV), which is defined by the multiplication of a sparse matrix by a dense vector to produce a dense output vector. It is generally considered the most predictable operation because the output size is known beforehand, allowing for straightforward memory allocation. While the design of an interface should support for various input formats (e.g. by accepting ana abstract matrix operator), the operation is typically memory-bandwidth limited and trivial to parallelize by partitioning the matrix rows among different execution units. 


In contrast, \textbf{Sparse Matrix-Matrix Multiplication} (SpGEMM) represents a much more complex archetype due to the unknown output sparsity pattern. Because the number of non-zero entries in the resulting matrix cannot be determined a priori, a multi-stage design design is needed. This process begins with an inspection or symbolic phase to compute the sparsity pattern, followed by memory allocation, and finally a numeric phase to calculate the actual values. Because the workload can be highly unbalanced, the parallelization strategies for SpGEMM can range from row-parallel and column-parallel to sub-matrix parallel and element-parallel.
...
\subsection{Multi-Stage API for Operations with unknown Output}
