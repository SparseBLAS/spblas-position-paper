...
In \Cref{fig:csrmatrixhandle} we demonstrate how the ``matrix\_view'' wraps a user-owned CSR matrix structure, then encapsulates the ``matrix\_view'' into a matrix\_handle. 


Generally speaking, any sparse BLAS functionality should take matrix\_handles as the input arguments and create matrix\_views as output arguments - if creating sparse matrices as output.
\todo{How about a masking operation? Is the input mask a matrix\_handle or a matrix\_opt? Where is the output stored in?}
Furthermore, any change to a matrix\_view that is wrapped into a matrix\_handle has to be propagated explicitly to the matrix\_handle, as any library-owned optimization would otherwise not know about the changes in the matrix\_view. Conversely, only a few sparse BLAS functionalities change the matrix\_view underlying a matrix\_handle input argument. (These include sorting, transposing, etc.) 

\begin{listing}[H]
\begin{minted}{c++}
using namespace sparseblas;
csr_view<float> A(values, rowptr, colind, shape, nnz, tags);
...
Libraries are free to implement views however they see fit. For instance, a simple masked access works well for lower triangular matrices in matrix-vector products. However, for transposed operations, it may be better to store an explicit copy in the matrix\_handle to optimize performance.
